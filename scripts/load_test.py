...
def send_request(client: httpx.Client, payload: dict) -> None:
    try:
        ...
        print(
            f"[{r.status_code}] {r.json().get('correlation_id')} | {payload['feature']} | {latency:.1f}ms")
...
def main() -> None:
    ...
    lines = [line for line in QUERIES.read_text(
        ...
        if args.concurrency > 1:
            with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as executor:
                futures = [executor.submit(
                    send_request, client, json.loads(line)) for line in lines]
